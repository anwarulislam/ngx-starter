...
export class AuthService {
    ...
    register(credentials): Observable<User> {
        return this.http.post(this.routes.register, credentials)
            .pipe(
                map((user: User) => {
                    this.createSession(user);
                    this.redirectToReturnUrl();
    ...
    recoverPassword(credentials): Observable<User> {
        return this.http.post(this.routes.recover, credentials)
            .pipe(map((res: any) => res?.data));
    }
